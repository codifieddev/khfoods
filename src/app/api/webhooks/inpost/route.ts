import { updateStorefrontOrderStatusByPackageNumber } from "@/data/storefront/commerce";
...
export async function POST(req: Request) {
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      console.log("Inpost Webhook received:", data);
    }

    if (data.event === "shipment_status_changed") {
      switch (data.payload.status) {
        case "confirmed": {
          await updateStorefrontOrderStatusByPackageNumber({
            packageNumber: data.payload.shipment_id,
            status: "processing",
