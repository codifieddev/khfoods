    ...
    <section className="w-full bg-white py-16">
      <div className="w-full max-w-[1440px] mx-auto px-6 md:px-12 lg:px-20">
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-10 items-stretch">
          ...
          {/* RIGHT PRODUCTS GRID (UPDATED) */}
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {products.map((product) => (
              <Link key={product.id} href="/products/single" className="block">
                {/* ✅ Image full width and content over image */}
                <article className="group relative rounded-2xl overflow-hidden bg-[#f7f7f7] h-[350px] sm:h-[350px] cursor-pointer">
                  {/* FULL IMAGE */}
                  <div className="absolute inset-0">
                    <img
                      src={product.img}
