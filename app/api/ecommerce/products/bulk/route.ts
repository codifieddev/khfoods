...
//        if (c.slug && c.name) {
//           validCategoryNamesMap.set(c.name.toLowerCase(), c.slug);
//        }
//     });
...
//       // Handle attribute sets and options mapping
//       const options = (p.options || []).map((option: any) => {
//         // Find which attribute set this option belongs to
//         const attrSet = allAttrSets.find(s =>
//           s.attributes && s.attributes.some((a: any) => a.name === option.label)
//         );
//         const attribute = attrSet?.attributes?.find((a: any) => a.name === option.label);

//         return {
//           attributeSetId: attrSet?.key || attrSet?._id?.toString(),
//           key: option.key || attribute?.key,
//           label: option.label,
//           values: attribute?.options || option.values || [],
//           selectedValues: option.values || [],
//           useForVariants: option.useForVariants !== undefined ? option.useForVariants : true
//         };
//       });
...
//               sku: v.sku,
//               title: v.title,
//               price: v.price !== undefined ? v.price : (p.price || 0),
//               stock: v.stock || 0,
...
//     return NextResponse.json({
//       success: true,
//       message: `${totalInsertedProducts} products and ${totalInsertedVariants} variants imported successfully`,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const attributeSetIds = await attributeSetColl.find({}).toArray();
